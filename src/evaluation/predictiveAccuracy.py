...
def cross_validate(ratings, recommender):
    ...
    _chunks = lambda lst, sz: [lst[i:i+sz] for i in range(0, len(lst), sz)]
    ...
    for i in range(1, num_folds):
        ...
        for rating in fold:
            prediction = recommender.get_rating(rating.user, rating.item)
            pairs.append((rating.value, prediction))

        queue.append(fold)

    # return average error
    return mean_absolute_error(pairs, range(1, 5))
